...
class BaseOptions():
    ...
    def gather_options(self):
        ...
        if not self.initialized:  # check if it has been initialized
            parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)  # 建立解析对象
            parser = self.initialize(parser)

        # 获得基本选项
        opt, _ = parser.parse_known_args()  # 接收到多余命令参数时不报错

        # 添加与模型相关的解析器选项
        model_name = opt.model  # 获得模型名称
        ...
        # 保存并将解析器返回
        self.parser = parser
        return parser.parse_args()  # 将parser中设置的所有"add_argument"返回， 那么parser中增加的属性内容都会在返回的参数中，使用即可
    ...
    def print_options(self, opt):
        ...
        本代码段将打印 当前选项 与 默认值（如果二者不同）
        It will save options into a text file / [checkpoints_dir] / opt.txt
        本代码段将会保存选项到一个txt文件，路径为 ‘/ [checkpoints_dir] / opt.txt’
        """
        message = ''
        message += '----------------- Options ---------------\n'
